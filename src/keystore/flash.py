    ...
    async def storage_menu(self, title="Manage keys on internal flash"):
        ...
        while True:
            ...
            if menuitem == 255:
                ...
                if filename:
                    await self.show(
                        Alert(t("Success!"), t("Your key is stored now.\n\nName: %s") % filename, button_text=t("OK"))
                    )
            elif menuitem == 1:
                if await self.load_mnemonic():
                    await self.show(
                        Alert(t("Success!"), t("Your key is loaded."), button_text=t("OK"))
                    )
                return True
            elif menuitem == 2:
                if await self.delete_mnemonic():
                    await self.show(
                        Alert(t("Success!"), t("Your key is deleted."), button_text=t("OK"))
                    )
